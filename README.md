...
- Container access via `$app->getCompatibilityContainer()` works for Slim 3 style access:
  - `$app->getCompatibilityContainer()->get('router')` - access to router with `urlFor()` and `getRoutes()`
  - `$app->getCompatibilityContainer()['settings']` - access to settings
